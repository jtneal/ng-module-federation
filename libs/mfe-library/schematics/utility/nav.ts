...
export function updateNavTemplate(applications: Application[], sourceRoot: string): Rule {
  return (host: Tree): Tree => {
    const htmlPath = normalize(`${sourceRoot}/app/micro-frontend-nav/micro-frontend-nav.component.html`);
    const htmlData = `<nav>
  <ul>
    ${
      applications
...
export function addNavToAppComponent(sourceRoot: string): Rule {
  return (host: Tree): Tree => {
    ...
    const htmlPath = normalize(`${sourceRoot}/app/app.component.html`);
